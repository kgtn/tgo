...
import { X, Bot, ShieldCheck, Check, Plus, Loader2, Wrench } from 'lucide-react';
    ...
    const fetchToolsDetail = async () => {
      ...
      try {
        const tools = await Promise.all(
          agent.recommended_tools.map(id => storeApi.getTool(id))
        );
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {toolsDetail.map(tool => (
                        <div 
                          ...
                          className="group p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-indigo-200 dark:hover:border-indigo-900 hover:bg-white dark:hover:bg-gray-800 transition-all cursor-pointer flex items-center gap-3 shadow-sm hover:shadow-md"
                        >
                          <div className="w-10 h-10 rounded-lg bg-white dark:bg-gray-900 flex items-center justify-center text-xl border border-gray-100 dark:border-gray-800 group-hover:scale-110 transition-transform">
                            {tool.icon || '🛠️'}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-[10px] text-gray-400 font-medium truncate">
                              {currentLang === 'zh' ? (tool.description_zh || tool.description) : (tool.description_en || tool.description_zh || tool.description)}
                            </div>
                          </div>
                        </div>
                      ))}
